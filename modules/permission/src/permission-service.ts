...
export class PermissionService {
  ...
  #replace(current: Permission, context: Context, changes: Partial<Permission>, eventType: string, action: string, reason: string, details: Readonly<Record<string, unknown>> = {}): Permission { const nextStatus = changes.status ?? current.status; if (terminalStatuses.has(current.status)) throw new Error("STATE_TRANSITION_INVALID"); const updated = immutable<Permission>({ ...current, ...changes, version: current.version + 1, statusHistory: nextStatus === current.status ? current.statusHistory : [...current.statusHistory, { status: nextStatus, actorId: context.actor.principalId, occurredAt: this.#clock().toISOString(), reason }] }); this.#store(updated); this.#record(eventType, action, updated, context, details, reason); return updated; }
  ...
  #record(eventType: string, action: string, permission: Permission, context: Context, details: Readonly<Record<string, unknown>>, reason?: string): void { this.#audit.append({ eventType, principal: principal(context.actor), action, target: { type: "Permission", id: permission.id, version: permission.version }, purpose: context.purpose, policyVersion: this.#policyVersion, classification: permission.classification, decision: "ALLOW", outcome: "COMPLETED", ...(reason === undefined ? {} : { reason }), ...(context.requestId === undefined ? {} : { requestId: context.requestId }), correlationId: context.correlationId, details: { ...details, teamId: permission.teamId, subjectType: permission.subjectRef.entityType, subjectVersion: permission.subjectRef.version, rawContactIncluded: false, evidenceHandling: "REFERENCE_ONLY" } }); }
}
